...
export type CrtInput = {
  ...
  score?: number;
  gameOver?: boolean;
  gems?: { x: number; collected: boolean }[];
  crates?: { x: number }[];
};

export type ScreenPainter = (context: CanvasRenderingContext2D, width: number, height: number, time: number, input: CrtInput) => void;
...
const paintCinematic: ScreenPainter = (context, width, height, time, input) => {
...
const paintBlueScreen: ScreenPainter = (context, width, height, time, input) => {
...
const paintNintendo: ScreenPainter = (context, width, height, time, input) => {
  const groundY = GROUND_ROW;
  const isPlaying = input.active && !input.gameOver;
  ...
  if (!isPlaying) {
    const phase = ((time % NINTENDO_CYCLE) + NINTENDO_CYCLE) % NINTENDO_CYCLE;
    for (const crateX of CRATE_X) { drawCrate(context, crateX, groundY - 16); drawGem(context, crateX + 8, groundY - 65 - bob, spin); }
    ...
    context.save(); context.beginPath(); context.rect(sweepX, 50, 24, 21); context.clip();
    drawPixelText(context, title, titleX, 50, 3, "#fffce0"); context.restore();
    ...
    const joinText = "TIKTOK.COM/@_THANHWI";
    const joinX = width - pixelTextWidth(joinText, 1) - 10;
    // Highlighted TikTok Link (Title Screen)
    context.fillStyle = "rgba(0, 0, 0, 0.7)";
    context.fillRect(joinX - 4, 3, pixelTextWidth(joinText, 1) + 8, 12);
    drawPixelText(context, joinText, joinX, 7, 1, "#000000");
    drawPixelText(context, joinText, joinX, 6, 1, "#ffff00");

    if (input.gameOver) {
      const msg = "GAME OVER", msgX = Math.round((width - pixelTextWidth(msg, 2)) / 2);
      drawPixelText(context, msg, msgX + 2, 100, 2, "#141028");
      drawPixelText(context, msg, msgX, 98, 2, "#f42c2c");
      const sub = `SCORE: ${input.score}`, subX = Math.round((width - pixelTextWidth(sub, 1)) / 2);
      drawPixelText(context, sub, subX, 120, 1, "#ffffff");
      if (Math.floor(time * 2) % 2 === 0) {
        const retry = "CLICK TO RESTART", retryX = Math.round((width - pixelTextWidth(retry, 1)) / 2);
        drawPixelText(context, retry, retryX, 140, 1, "#ffffff");
      }
    } else if (Math.floor(time * 1.6) % 2 === 0) {
      const start = "CLICK TO START", startX = Math.round((width - pixelTextWidth(start, 2)) / 2);
    ...
    // Playable mode logic
    const runX = 60;
    const lift = input.jumpTime ?? 0;
    const spin = Math.floor(time * 8) % 4, bob = [0, 1, 1, 0][Math.floor(time * 6) % 4];
    
    // Draw crates from input
    if (input.crates) {
      for (const crate of input.crates) {
        drawCrate(context, crate.x, groundY - 16);
      }
    }

    // Draw gems from input
    if (input.gems) {
      for (const gem of input.gems) {
        if (!gem.collected) {
          drawGem(context, gem.x + 8, groundY - 85 - bob, spin);
        }
      }
    }

    const pose = lift > 0.5 ? "jump" : Math.floor(time * 12) % 2 === 0 ? "a" : "b";
    context.fillStyle = "rgba(20,16,10,0.20)"; context.fillRect(runX + 2, groundY - 1, 10, 2);
    drawRunner(context, runX, groundY - 16 - Math.round(lift), pose, false);

    // UI
    const scoreText = `SCORE ${pad(input.score ?? 0, 5)}`;
    drawPixelText(context, scoreText, 10, 7, 1, "#141428");
    drawPixelText(context, scoreText, 10, 6, 1, "#ffffff");

    const joinText = "TIKTOK.COM/@_THANHWI";
    const joinX = width - pixelTextWidth(joinText, 1) - 10;
    // Highlighted TikTok Link (Game Screen)
    context.fillStyle = "rgba(0, 0, 0, 0.7)";
    context.fillRect(joinX - 4, 3, pixelTextWidth(joinText, 1) + 8, 12);
    drawPixelText(context, joinText, joinX, 7, 1, "#000000");
    drawPixelText(context, joinText, joinX, 6, 1, "#ffff00");
    
    drawPixelText(context, "WORLD 1-1", 174, 7, 1, "#141428");
    drawPixelText(context, "WORLD 1-1", 174, 6, 1, "#ffffff");

    const timeText = "TIME " + pad(Math.max(0, 300 - Math.floor(time)) % 1000, 3);
    drawPixelText(context, timeText, 254, 7, 1, "#141428");
    drawPixelText(context, timeText, 254, 6, 1, "#ffffff");
  }
};
